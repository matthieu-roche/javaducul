...
public class PetitRequin extends Poisson{
	...
	public void bouger2()
	{
						...
						if (or + (yv-or)*vit/norm > 0) { //Pas trop haut
							...
							if (ab + (xv-ab)*vit/norm == voisin.getAbs()) { // si l'abscisse est la meme
								if (!(or + (yv-or)*vit/norm == voisin.getOrd())) { // il faut que l'ordonnée ne soit pas la même
									...
									System.out.println("Il est déja très proche de son copain Petit Requin"); // sinon c'est qu'ils sont déja assez proche
								}
							}
							else { // si l'abscisse n'est pas la même
								System.out.println("Il s'approche de son copain Petit Requin"); // il peut s'approcher pas de problèmes
								setAbs((int)(ab + (xv-ab)*vit/norm));
								setOrd((int)(or + (yv-or)*vit/norm));
							}	
						}
		...
		// Puis, les prédateurs YEN A PAS ICI
